// Recupera el carrito desde localStorage o lo inicializa vacío
...
const carritoContenido = carritoModal?.querySelector('.carrito-modal-content'); // Contenido del modal
...
function renderizarCarrito() {
    ...
    if (carrito.length === 0) {
        ...
        // Recorre cada producto en el carrito y lo muestra
        carrito.forEach((item, idx) => {
            // Convierte el precio a número (elimina símbolos)
        ...
        html += `</ul>`;
        // Muestra el total y el botón para ir a pagar
...
function agregarAlCarrito(img, nombre, precio) {
    ...
    if (existente) {
        existente.cantidad += 1; // Si ya existe, suma uno
    } else {
        carrito.push({ img, nombre, precio, cantidad: 1 }); // Si no existe, lo agrega
    }
    actualizarContador();
    renderizarCarrito();
}